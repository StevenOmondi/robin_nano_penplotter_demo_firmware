...
def hub_icon(name, draw_fn):
    # Draw on the classic 117x126 canvas (card + bottom label margin),
    # then crop the bottom margin so the final icon is 117x102. This keeps
    # the card and glyphs pixel-identical while fitting two rows under the
    # live status bar on the 320 px tall screen.
    w, h, h_out = 117, 126, 102
    img, d = canvas(w, h)
    d.rounded_rectangle(
    ...
    draw_fn(img, d)
    img = img.crop((0, 0, w * SS, h_out * SS))
    finish(img, w, h_out, os.path.join(OUT, name))
