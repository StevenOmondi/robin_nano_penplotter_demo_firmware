...
typedef struct SettingsDataStruct {
  ...
  #if ENABLED(PENPLOTTER_EMBEDDED_DEMOS)
    PenPlotterSettings penplotter_settings_data;        // Z pen lift/down and favorite demo
  #endif
...
#include "../core/debug_out.h"
  ...
  bool MarlinSettings::save() {
    ...
    #if ENABLED(PENPLOTTER_EMBEDDED_DEMOS)
      _FIELD_TEST(penplotter_settings_data);
      EEPROM_WRITE(penplotter_settings);
    #endif
  ...
  EEPROM_Error MarlinSettings::_load() {
    ...
    do { // A block to break out of on error
      ...
      #if ENABLED(PENPLOTTER_EMBEDDED_DEMOS)
        _FIELD_TEST(penplotter_settings_data);
        EEPROM_READ(penplotter_settings);
        if (!validating) penplotter_settings_sanitize();
      #endif
...
void MarlinSettings::reset() {
  ...
  //
  TERN_(PENPLOTTER_EMBEDDED_DEMOS, penplotter_settings_reset());
  ...
  void MarlinSettings::report(const bool forReplay) {
    ...
    #if ENABLED(PENPLOTTER_EMBEDDED_DEMOS)
      CONFIG_ECHO_HEADING("Pen Plotter");
      CONFIG_ECHO_START();
      SERIAL_ECHOPAIR_F("  M751 I", penplotter_settings.pen_up_z, 2);
      SERIAL_ECHOLNPAIR_F(" K", penplotter_settings.pen_down_z, 2);
    #endif
