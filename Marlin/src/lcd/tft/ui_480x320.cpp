...
#if !HAS_EXTRUDERS
  static void draw_plotter_home_button(const uint16_t x, const uint16_t y, const uint16_t w, const uint16_t h, const char * const title, const char * const detail, const uint16_t color) {
    tft.fill(x, y, w, h, COLOR_DARKGREY2);
    tft.canvas(x, y, w, h);
    tft.set_background(COLOR_DARKGREY2);
    tft.add_bar(0, 0, w, 5, color);
    tft.add_rectangle(0, 0, w, h, COLOR_DARKGREY);

    const uint16_t title_y = h < 56 ? 5 : 11,
                   detail_y = h < 56 ? 25 : 40;

    tft_string.set(title);
    tft_string.trim();
    tft.add_text(16, title_y, COLOR_YELLOW, tft_string);

    if (detail && detail[0]) {
      tft_string.set(detail);
      tft_string.trim();
      tft.add_text(16, detail_y, COLOR_WHITE, tft_string);
    }
  }

  static void draw_plotter_action_button(const uint16_t x, const uint16_t y, const uint16_t w, const uint16_t h, const char * const label, const intptr_t data, const uint16_t color, const bool enabled=true) {
    const uint16_t draw_color = enabled ? color : COLOR_GREY;
    tft.fill(x, y, w, h, COLOR_DARKGREY2);
    tft.canvas(x, y, w, h);
    tft.set_background(COLOR_DARKGREY2);
    tft.add_bar(0, 0, w, 5, draw_color);
    tft.add_rectangle(0, 0, w, h, COLOR_DARKGREY);

    tft_string.set(label);
    tft_string.trim();
    tft.add_text(tft_string.center(w), h / 2 - tft_string.font_height() / 2 + 2, enabled ? COLOR_YELLOW : COLOR_GREY, tft_string);
    TERN_(HAS_TFT_XPT2046, if (enabled) touch.add_control(BUTTON, x, y, w, h, data));
  }
#endif
...
void MarlinUI::draw_status_screen() {
  ...
  #if !HAS_EXTRUDERS
  {
    ...
    tft.fill(0, 0, TFT_WIDTH, 44, COLOR_DODGER_BLUE);
    tft_string.set(F("SHAKESBEARD"));
    tft.add_text(14, 10, COLOR_YELLOW, tft_string);
    tft_string.set(F("260x380"));
    tft.add_text(TFT_WIDTH - 96, 10, COLOR_WHITE, tft_string);

    tft.fill(12, 56, TFT_WIDTH - 24, 42, COLOR_DARKGREY2);
    tft.canvas(12, 56, TFT_WIDTH - 24, 42);
    tft.set_background(COLOR_DARKGREY2);
    tft.add_text(18, 10, COLOR_AQUA, "X");
    const bool nhx = axis_should_home(X_AXIS);
    tft_string.set(blink && nhx ? "?" : ftostr4sign(LOGICAL_X_POSITION(current_position.x)));
    tft.add_text(48, 10, nhx ? COLOR_AXIS_NOT_HOMED : COLOR_WHITE, tft_string);

    tft.add_text(174, 10, COLOR_AQUA, "Y");
    const bool nhy = axis_should_home(Y_AXIS);
    tft_string.set(blink && nhy ? "?" : ftostr4sign(LOGICAL_Y_POSITION(current_position.y)));
    tft.add_text(204, 10, nhy ? COLOR_AXIS_NOT_HOMED : COLOR_WHITE, tft_string);

    tft.add_text(330, 10, COLOR_AQUA, "Z");
    const bool nhz = axis_should_home(Z_AXIS);
    tft_string.set(blink && nhz ? "?" : ftostr52sp(LOGICAL_Z_POSITION(current_position.z)));
    tft_string.rtrim();
    tft.add_text(360, 10, nhz ? COLOR_AXIS_NOT_HOMED : COLOR_WHITE, tft_string);

    draw_plotter_home_button(12, 116, 218, 72, "JOG", "Move axes", COLOR_DODGER_BLUE);
    draw_plotter_home_button(250, 116, 218, 72, "PEN", "Servo + setup", COLOR_ORANGE);
    #if HAS_MEDIA
      draw_plotter_home_button(12, 204, 218, 72, "FILES", "SD draw jobs", COLOR_AQUA);
    #else
      draw_plotter_home_button(12, 204, 218, 72, "FILES", "No SD", COLOR_GREY);
    #endif
    draw_plotter_home_button(250, 204, 218, 72, "AREA", "260 x 380 mm", COLOR_VIVID_GREEN);

    TERN_(TOUCH_SCREEN, touch.add_control(MOVE_AXIS, 12, 56, TFT_WIDTH - 24, 42));
    TERN_(TOUCH_SCREEN, touch.add_control(MOVE_AXIS, 12, 116, 218, 72));
    TERN_(TOUCH_SCREEN, touch.add_control(MENU_SCREEN, 250, 116, 218, 72, (intptr_t)menu_main));
    #if HAS_MEDIA
      TERN_(TOUCH_SCREEN, touch.add_control(MENU_SCREEN, 12, 204, 218, 72, (intptr_t)menu_media));
    #endif
    TERN_(TOUCH_SCREEN, touch.add_control(MENU_SCREEN, 250, 204, 218, 72, (intptr_t)menu_main));

    tft.canvas(0, TFT_HEIGHT - FONT_LINE_HEIGHT, TFT_WIDTH, FONT_LINE_HEIGHT);
    tft.set_background(COLOR_BACKGROUND);
    tft_string.set(status_message);
    tft_string.trim();
  ...
  if (TERN0(LCD_SHOW_E_TOTAL, printingIsActive())) {
    ...
    const bool nhx = axis_should_home(X_AXIS);
    tft_string.set(blink && nhx ? "?" : ftostr4sign(LOGICAL_X_POSITION(current_position.x)));
    tft.add_text(102 - tft_string.width(), 3, nhx ? COLOR_AXIS_NOT_HOMED : COLOR_WHITE, tft_string);

    tft.add_text(192, 3, COLOR_AXIS_HOMED , "Y");
    const bool nhy = axis_should_home(Y_AXIS);
    tft_string.set(blink && nhy ? "?" : ftostr4sign(LOGICAL_Y_POSITION(current_position.y)));
    tft.add_text(280 - tft_string.width(), 3, nhy ? COLOR_AXIS_NOT_HOMED : COLOR_WHITE, tft_string);
  }
  ...
  tft.add_text(455 - tft_string.width() - offset, 3, nhz ? COLOR_AXIS_NOT_HOMED : COLOR_WHITE, tft_string);
...
static void pen_up() {
  quick_feedback();
  queue.inject(F("M280 P0 S30\nM117 Pen up"));
}

static void pen_down() {
  quick_feedback();
  queue.inject(F("M280 P0 S90\nM117 Pen down"));
}
...
void MarlinUI::move_axis_screen() {
  ...
  #if !HAS_EXTRUDERS
  {
    ...
    tft.fill(0, 0, TFT_WIDTH, TFT_HEIGHT, COLOR_BACKGROUND);
    tft.fill(0, 0, TFT_WIDTH, 40, COLOR_DODGER_BLUE);
    tft_string.set(F("SHAKESBEARD JOG"));
    tft.add_text(14, 8, COLOR_YELLOW, tft_string);
    tft_string.set(F("260x380"));
    tft.add_text(TFT_WIDTH - 96, 8, COLOR_WHITE, tft_string);

    tft.fill(12, 48, TFT_WIDTH - 24, 32, COLOR_DARKGREY2);
    tft.add_text(22, 55, X_BTN_COLOR, "X");
    motionAxisState.xValuePos.x = 50;
    motionAxisState.xValuePos.y = 53;
    drawAxisValue(X_AXIS);
    tft.add_text(174, 55, Y_BTN_COLOR, "Y");
    motionAxisState.yValuePos.x = 202;
    motionAxisState.yValuePos.y = 53;
    drawAxisValue(Y_AXIS);
    tft.add_text(326, 55, Z_BTN_COLOR, "Z");
    motionAxisState.zValuePos.x = 354;
    motionAxisState.zValuePos.y = 53;
    drawAxisValue(Z_AXIS);

    draw_plotter_action_button(center_x, 104, button_w, button_h, "Y+", (intptr_t)y_plus, Y_BTN_COLOR, !busy);
    draw_plotter_action_button(action_x, 104, action_w, button_h, "HOME", (intptr_t)do_home, COLOR_CONTROL_CONFIRM, !busy);

    draw_plotter_action_button(left_x, 156, button_w, button_h, "X-", (intptr_t)x_minus, X_BTN_COLOR, !busy);
    draw_plotter_action_button(right_x, 156, button_w, button_h, "X+", (intptr_t)x_plus, X_BTN_COLOR, !busy);
    draw_plotter_action_button(action_x, 156, action_w, button_h, "MOTOR", (intptr_t)enable_steppers, COLOR_CONTROL_CONFIRM, !busy);

    draw_plotter_action_button(center_x, 208, button_w, button_h, "Y-", (intptr_t)y_minus, Y_BTN_COLOR, !busy);
    draw_plotter_action_button(action_x, 208, action_w, button_h, "OFF", (intptr_t)disable_steppers, COLOR_CONTROL_CANCEL, !busy);

    motionAxisState.stepValuePos.x = left_x;
    motionAxisState.stepValuePos.y = 264;
    tft.fill(left_x, 264, CUR_STEP_VALUE_WIDTH, button_h, COLOR_DARKGREY2);
    drawCurStepValue();
    TERN_(HAS_TFT_XPT2046, if (!busy) touch.add_control(BUTTON, left_x, 264, CUR_STEP_VALUE_WIDTH, button_h, (intptr_t)step_size));

    draw_plotter_action_button(146, 264, 82, button_h, "UP", (intptr_t)pen_up, COLOR_ORANGE, !busy);
    draw_plotter_action_button(240, 264, 82, button_h, "DOWN", (intptr_t)pen_down, COLOR_ORANGE, !busy);
    draw_plotter_home_button(action_x, 264, action_w, button_h, "BACK", "", COLOR_SILVER);
    TERN_(HAS_TFT_XPT2046, touch.add_control(BACK, action_x, 264, action_w, button_h));
    return;
  }
  #endif
