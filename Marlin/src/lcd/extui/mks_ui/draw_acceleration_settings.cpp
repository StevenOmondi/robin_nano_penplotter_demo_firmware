...
void lv_draw_acceleration_settings() {
  ...
  if (!uiCfg.para_ui_page) {
    ...
    lv_screen_menu_item_1_edit(scr, machine_menu.TravelAcceleration, PARA_UI_POS_X, y, event_handler, ID_ACCE_TRAVEL, 1, public_buf_l);

    y += PARA_UI_POS_Y;
    #if HAS_X_AXIS
      itoa(planner.settings.max_acceleration_mm_per_s2[X_AXIS], public_buf_l, 10);
      lv_screen_menu_item_1_edit(scr, machine_menu.X_Acceleration, PARA_UI_POS_X, y, event_handler, ID_ACCE_X, 2, public_buf_l);
    #endif
    ...
    #if HAS_MULTI_EXTRUDER && ENABLED(DISTINCT_E_FACTORS)
      y += PARA_UI_POS_Y;
