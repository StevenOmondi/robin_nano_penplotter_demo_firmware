...
extern lv_group_t *g;
static lv_obj_t *scr;
static lv_obj_t *labelXpos, *labelYpos, *labelZpos, *labelSpeed, *labelTime;
...
void lv_draw_printing() {
  ...
  labelXpos  = lv_label_create(scr, 220, 136, nullptr);
  labelYpos  = lv_label_create(scr, 220, 166, nullptr);
  labelZpos  = lv_label_create(scr, 220, 196, nullptr);
  labelSpeed = lv_label_create(scr, 350, 136, nullptr);
...
void disp_ext_temp() {
  char pos[16];
  dtostrf(current_position.x, 1, 3, pos);
  sprintf_P(public_buf_l, PSTR("X:%s"), pos);
  lv_label_set_text(labelXpos, public_buf_l);
}

void disp_bed_temp() {
  char pos[16];
  dtostrf(current_position.y, 1, 3, pos);
  sprintf_P(public_buf_l, PSTR("Y:%s"), pos);
  lv_label_set_text(labelYpos, public_buf_l);
}

void disp_fan_speed() {
  sprintf_P(public_buf_l, PSTR("Speed:%d%%"), feedrate_percentage);
  lv_label_set_text(labelSpeed, public_buf_l);
}
...
void disp_fan_Zpos() {
  char pos[16];
  dtostrf(current_position.z, 1, 3, pos);
  sprintf_P(public_buf_l, PSTR("Z:%s"), pos);
