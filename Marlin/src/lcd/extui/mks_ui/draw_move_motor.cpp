...
static void event_handler(lv_obj_t *obj, lv_event_t event) {
  ...
  if (!queue.ring_buffer.full(4)) {
    ...
    if (do_inject) {
      sprintf_P(public_buf_l, PSTR("G1 %c%s F%d"), cur_label, dtostrf(dist, 1, 3, str_1), uiCfg.moveSpeed);
      queue.enqueue_one(F("M751 A"));
      queue.enqueue_one(F("G91"));
      queue.enqueue_one(public_buf_l);
      queue.enqueue_one(F("G90"));
    }
  }
